...
class PhaseSpaceCoordinate(AbstractBasicPhaseSpaceCoordinate):
    ...
    t: gt.BBtQuSz0 | gt.SzN | gt.QuSz0 = eqx.field(converter=u.Quantity["time"].from_)
    """The time corresponding to the positions.
...
def from_(
    ...
    PhaseSpaceCoordinate(
      q=CartesianPos3D( ... ),
      p=CartesianVel3D( ... ),
      t=Quantity(Array([7, 7], dtype=int64, ...), unit='Myr'),
      frame=SimulationFrame()
    )
...
def from_(
    ...
    PhaseSpaceCoordinate(
      q=CartesianPos3D( ... ),
      p=CartesianVel3D( ... ),
      t=Quantity(Array(0., dtype=float64, ...), unit='kpc s / km'),
      frame=SimulationFrame()
    )
