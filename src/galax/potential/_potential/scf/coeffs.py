...
from galax.typing import FloatLike, IntLike
...
def compute_coeffs_discrete(
    ...
    *,
    ...
    ns = xp.arange(nmax + 1)[:, None]  # (n, l)
    ls = xp.arange(lmax + 1)[None, :]  # (n, l)

    Anl_til = expansion_coeffs_Anl_discrete(ns, ls)  # (n, l)
    phinl = phi_nl_vec(s, ns, ls, ggncalc)  # (n, l, N)

    li, mi = xp.tril_indices(lmax + 1)  # (l*(l+1)//2,)
    lm = xp.zeros((lmax + 1, lmax + 1), dtype=int).at[li, mi].set(li)  # (l, m)
    ms = xp.zeros((lmax + 1, lmax + 1), dtype=int).at[li, mi].set(mi)  # (l, m)
    # TODO: this is VERY SLOW. Can we do better?
    Ylm = real_Ylm(theta, lm, ms, m_max=100)  # (l, m, N)

    delta = jax.lax.select(ms == 0, xp.ones_like(ms), xp.zeros_like(ms))  # (l, m)
    mvalid = xp.zeros((lmax + 1, lmax + 1)).at[li, mi].set(1)  # select m <= l
    ...
    Snlm = xp.sum(tmp * xp.cos(ms[None, :, :, None] * phi[None, None, None]), axis=-1)
    Tnlm = xp.sum(tmp * xp.sin(ms[None, :, :, None] * phi[None, None, None]), axis=-1)
