...
from galax.typing import ArrayAnyShape, BatchableIntLike, IntLike, Vec3, VecN
from galax.utils import partial_jit, partial_vectorize

T = TypeVar("T", bound=ArrayLike)


@jax.jit  # type: ignore[misc]
@partial_vectorize(signature="(3)->(3)")
def cartesian_to_spherical(xyz: Vec3, /) -> Vec3:
    ...
    """
    r = xp.sqrt(xp.sum(xyz**2, axis=0))  # spherical radius
    # TODO: this is a hack to avoid the ambiguity at r==0. This should be done better.
    theta = jax.lax.select(
        r == 0, xp.zeros_like(r), xp.arccos(xyz[2] / r)
    )  # inclination angle
    phi = xp.arctan2(xyz[1], xyz[0])  # azimuthal angle
    return xp.array([r, theta, phi])
...
@partial_vectorize(signature="(n),(),()->(n)", excluded=(3,))
@partial_jit(static_argnames=("m_max",))  # TODO: should l,m be static?
def _real_Ylm(theta: VecN, l: IntLike, m: IntLike, m_max: int) -> VecN:
...
@partial_jit(static_argnames=("m_max",))
def real_Ylm(
    theta: ArrayAnyShape, l: BatchableIntLike, m: BatchableIntLike, m_max: int = 100
) -> ArrayAnyShape:
