...
class LeeSutoTriaxialNFWPotential(AbstractPotential):
    ...
    def _potential(  # TODO: inputs w/ units
        ...
        out: gt.BtFloatQScalar = phi0 * qlax.select(
            u == 0,
            jnp.ones_like(u),
            (
                F1
...
class TriaxialNFWPotential(AbstractPotential):
    ...
    def rho0(self, t: gt.BBtRealQScalar, /) -> gt.BtFloatQScalar:
        r"""Central density.
...
class Vogelsberger08TriaxialNFWPotential(AbstractPotential):
    ...
    @partial(jax.jit, inline=True)
    def _r_e(self, q: gt.BtQVec3, t: gt.BBtRealQScalar) -> gt.BtFloatQScalar:
        q1sq = self.q1(t) ** 2
        q2sq = 3 - q1sq
        return jnp.sqrt(q[..., 0] ** 2 + q[..., 1] ** 2 / q1sq + q[..., 2] ** 2 / q2sq)

    @partial(jax.jit, inline=True)
    def _r_tilde(self, q: gt.BtQVec3, t: gt.BBtRealQScalar) -> gt.BtFloatQScalar:
