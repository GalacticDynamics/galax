...
from plum import Dispatcher, convert, dispatch

import coordinax as cx
import coordinax.frames as cxf
import quaxed.numpy as jnp
import unxt as u
from unxt.quantity import AbstractQuantity, BareQuantity
...
) -> tuple[gt.BBtRealSz3, gt.BBtRealSz0]:
    """Parse input arguments to position & time."""
...
@coord_dispatcher.multi(
    (cxf.AbstractReferenceFrame | None, gt.BBtRealQuSz3, gt.BBtRealQuSz0),
    (cxf.AbstractReferenceFrame | None, gt.BBtRealQuSz3, gt.BBtRealSz0 | float | int),
)
def parse_to_xyz_t(
    to_frame: cxf.AbstractReferenceFrame | None,
...
) -> tuple[gt.BBtRealQuSz3 | gt.BBtRealSz3, gt.BBtRealQuSz0 | gt.BBtRealSz0]:
    """Parse input arguments to position & time."""
    xyz = jnp.asarray(xyz, dtype=dtype)
    t = jnp.asarray(t, dtype=dtype)
...
@dispatch
def parse_to_quantity_or_array(
    coord: cxf.AbstractCoordinate, /, **kw: Any
) -> gt.BtRealQuSz3:
