...
from galax.potential._potential.scf.bfe_helper import rho_nl
from galax.potential._potential.scf.coeffs_helper import (
...
def compare_rho_nl(s, n, l):
    ...
    return -xp.sum((observed - mock) ** 2)


@pytest.mark.skip(reason="TODO")
def test_rho_nl():
    """Test the ``rho_nl`` function."""
    s = xp.linspace(0, 4, 100, dtype=float)
    n = xp.array([1.0])
    l = xp.array([2.0])
