...
class ParameterPhiMixin(ParameterFieldMixin):
    ...
    def test_phi_userfunc(self, pot_cls, fields):
        ...
        pot = pot_cls(**fields)
        assert pot.phi(t=u.Quantity(0, "Myr")) == 2
...
class TestLMJ09LogarithmicPotential(
    ...
    def test_potential(self, pot: LMJ09LogarithmicPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(0.11819267, unit="kpc2 / Myr2")
        assert jnp.isclose(
            pot.potential(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_gradient(self, pot: LMJ09LogarithmicPotential, x: gt.QSz3) -> None:
        expect = u.Quantity([-0.00046885, 0.00181093, 0.00569646], "kpc / Myr2")
        got = convert(pot.gradient(x, t=0), u.Quantity)
        assert jnp.allclose(got, expect, atol=u.Quantity(1e-8, expect.unit))

    def test_density(self, pot: LMJ09LogarithmicPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(48995543.34035844, "solMass / kpc3")
        assert jnp.isclose(
            pot.density(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_hessian(self, pot: LMJ09LogarithmicPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
            [
