...
class TestLogarithmicPotential(
    ...
    def test_potential(self, pot: LogarithmicPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(0.11027593, unit="kpc2 / Myr2")
        assert jnp.isclose(
            pot.potential(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_gradient(self, pot: LogarithmicPotential, x: gt.QSz3) -> None:
        expect = u.Quantity([0.00064902, 0.00129804, 0.00194706], "kpc / Myr2")
        got = convert(pot.gradient(x, t=0), u.Quantity)
        assert jnp.allclose(got, expect, atol=u.Quantity(1e-8, expect.unit))

    def test_density(self, pot: LogarithmicPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(30321621.61178864, "solMass / kpc3")
        assert jnp.isclose(
            pot.density(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_hessian(self, pot: LogarithmicPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
            [
