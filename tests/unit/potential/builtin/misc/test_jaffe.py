...
class TestJaffePotential(
    ...
    def test_potential(self, pot: JaffePotential, x: gt.QSz3) -> None:
        expect = u.Quantity(-1.06550653, unit="kpc2 / Myr2")
        assert jnp.isclose(
            pot.potential(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_gradient(self, pot: JaffePotential, x: gt.QSz3) -> None:
        expect = u.Quantity([0.06776567, 0.13553134, 0.20329701], "kpc / Myr2")
        got = convert(pot.gradient(x, t=0), u.Quantity)
        assert jnp.allclose(got, expect, atol=u.Quantity(1e-8, expect.unit))

    def test_density(self, pot: JaffePotential, x: gt.QSz3) -> None:
        expect = u.Quantity(2.52814372e08, "solMass / kpc3")
        assert jnp.isclose(
            pot.density(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_hessian(self, pot: JaffePotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
            [
