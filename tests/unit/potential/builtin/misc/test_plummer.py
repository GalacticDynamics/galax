...
class TestPlummerPotential(
    ...
    def test_potential(self, pot: PlummerPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(-1.16150826, unit="kpc2 / Myr2")
        assert jnp.isclose(
            pot.potential(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_gradient(self, pot: PlummerPotential, x: gt.QSz3) -> None:
        expect = u.Quantity([0.07743388, 0.15486777, 0.23230165], "kpc / Myr2")
        got = convert(pot.gradient(x, t=0), u.Quantity)
        assert jnp.allclose(got, expect, atol=u.Quantity(1e-8, expect.unit))

    def test_density(self, pot: PlummerPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(2.73957531e08, "solMass / kpc3")
        assert jnp.isclose(
            pot.density(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_hessian(self, pot: PlummerPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
            [
