...
class TestSatohPotential(
    ...
    def test_potential(self, pot: SatohPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(-0.97415472, unit="kpc2 / Myr2")
        assert jnp.isclose(
            pot.potential(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_gradient(self, pot: SatohPotential, x: gt.QSz3) -> None:
        expect = u.Quantity([0.0456823, 0.0913646, 0.18038493], "kpc / Myr2")
        got = convert(pot.gradient(x, t=0), u.Quantity)
        assert jnp.allclose(got, expect, atol=u.Quantity(1e-8, expect.unit))

    def test_density(self, pot: SatohPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(1.08825455e08, "solMass / kpc3")
        assert jnp.isclose(
            pot.density(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_hessian(self, pot: SatohPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
            [
