...
class TestTriaxialHernquistPotential(
    ...
    def test_potential(self, pot: TriaxialHernquistPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(-0.61215074, pot.units["specific energy"])
        assert jnp.isclose(
            pot.potential(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_gradient(self, pot: TriaxialHernquistPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
    ...
    def test_density(self, pot: TriaxialHernquistPotential, x: gt.QSz3) -> None:
        assert pot.density(x, t=0).decompose(pot.units).value >= 0

    def test_hessian(self, pot: TriaxialHernquistPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
            [
