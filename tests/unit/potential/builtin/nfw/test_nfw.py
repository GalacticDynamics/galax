...
class TestNFWPotential(
    ...
    def test_potential(self, pot: gp.NFWPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(-1.87120528, pot.units["specific energy"])
        assert jnp.isclose(
            pot.potential(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_gradient(self, pot: gp.NFWPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
    ...
    def test_density(self, pot: gp.NFWPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(9.45944763e08, pot.units["mass density"])
        assert jnp.isclose(
            pot.density(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_hessian(self, pot: gp.NFWPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
            [
