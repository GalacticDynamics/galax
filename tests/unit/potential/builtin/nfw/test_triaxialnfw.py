...
class TestTriaxialNFWPotential(
    ...
    def test_potential(self, pot: TriaxialNFWPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(-1.06475915, unit="kpc2 / Myr2")
        assert jnp.isclose(
            pot.potential(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_gradient(self, pot: TriaxialNFWPotential, x: gt.QSz3) -> None:
        expect = u.Quantity([0.03189139, 0.0604938, 0.13157674], "kpc / Myr2")
        got = convert(pot.gradient(x, t=0), u.Quantity)
        assert jnp.allclose(got, expect, atol=u.Quantity(1e-8, expect.unit))

    def test_density(self, pot: TriaxialNFWPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(2.32106514e08, "solMass / kpc3")
        assert jnp.isclose(
            pot.density(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_hessian(self, pot: TriaxialNFWPotential, x: gt.QSz3) -> None:
        expect = u.Quantity(
            [
