...
class TestBovyMWPotential2014(AbstractCompositePotential_Test):
    ...
    def test_potential(self, pot: gp.BovyMWPotential2014, x: gt.QSz3) -> None:
        expect = u.Quantity(-0.09550731, unit="kpc2 / Myr2")
        assert jnp.isclose(
            pot.potential(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_gradient(self, pot: gp.BovyMWPotential2014, x: gt.QSz3) -> None:
        expect = u.Quantity([0.00231875, 0.0046375, 0.01042675], "kpc / Myr2")
        got = convert(pot.gradient(x, t=0), u.Quantity)
        assert jnp.allclose(got, expect, atol=u.Quantity(1e-8, expect.unit))

    def test_density(self, pot: gp.BovyMWPotential2014, x: gt.QSz3) -> None:
        expect = u.Quantity(24_911_277.33877818, "solMass / kpc3")
        assert jnp.isclose(
            pot.density(x, t=0), expect, atol=u.Quantity(1e-8, expect.unit)
        )

    def test_hessian(self, pot: gp.BovyMWPotential2014, x: gt.QSz3) -> None:
        expect = u.Quantity(
            [
